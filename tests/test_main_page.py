...
class TestMainPage():
    ...
    @pytest.mark.xfail(reason='Тест так как при дате въезда позже даты выезда ожидается ошибка, но она не появляется')
    def test_set_check_in_date_later_than_check_out(self, open_main_page):
        browser, link = open_main_page
        page = MainPage(browser, link)
        page.scroll_to_header_check_dates()
        with allure.step('Результат: дата Check In = 15 числу будущего месяца'):
            page.set_calendar_date(date_type='check in', months_offset=1, day=15)
        with allure.step('Результат: дата Check Out = 10 числу будущего месяца'):
    ...
    @pytest.mark.xfail(reason='Тест так как при устаревших датах ожидается ошибка, но она не появляется')
    def test_set_past_dates(self, open_main_page):
        browser, link = open_main_page
        page = MainPage(browser, link)
        page.scroll_to_header_check_dates()
        with allure.step('Результат: дата Check In = 1 числу прошлого месяца'):
            page.set_calendar_date(date_type='check in', months_offset=-1, day=1)
        with allure.step('Результат: дата Check In = 3 числу прошлого месяца'):
